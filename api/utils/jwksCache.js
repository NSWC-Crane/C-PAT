...
class JWKSCache extends EventEmitter {
    ...
    request(url, options) {
        return new Promise((resolve, reject) => {
            ...
            const httpRequest = httpRequestLib.request(url, requestOptions, res => {
            ...
            function onSocket(socket) {
                ...
                socket.on('error', error => {
                    ...
                    logger.writeError('jwksCache', 'socketEvent', {
                        event: 'error',
                        socket: formatSocket(socketInfo),
                        message: err.message,
                    });
                });
                ...
                socket.on('connectionAttemptFailed', (ip, port, family, error) => {
                    socketInfo.remoteAddress = ip;
                    socketInfo.remotePort = port;
                    logger.writeDebug('jwksCache', 'socketEvent', {
            ...
            httpRequest.on('socket', onSocket).on('timeout', onTimeout).on('error', reject).end();
        });
    }
