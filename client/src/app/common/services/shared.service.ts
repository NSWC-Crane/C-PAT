...
export class SharedService {
  ...
  private async getSTIGManagerAuthHeaders() {
    const token = await firstValueFrom(
      this.oidcSecurityService.getAccessToken('stigman'),
    );
    return new HttpHeaders().set('Authorization', 'Bearer ' + token);
  }
  ...
  getApiConfig() {
    return this.http
      .get(`${this.cpatApiBase}/op/configuration`)
  ...
  async getSTIGMANAffectedAssetsByPoam(
    collectionId: string,
    benchmarkId: string,
  ) {
  ...
  async getAffectedAssetsFromSTIGMANByBenchmarkId(
    collectionId: string,
    benchmarkId: string,
  ) {
    const url = `${this.STIGMANAGER_URL}/collections/${collectionId}/stigs/${benchmarkId}/assets`;
