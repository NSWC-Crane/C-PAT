...
export class ImportService {
  ...
  async getTenablePlugin(pluginId: any): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/plugin/${pluginId}`, { headers })
      .pipe(catchError(this.handleError));
  }

  async getTenableAssetsFilter(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/asset?filter=excludeWatchlists,excludeAllDefined,usable&fields=name`, { headers })
      .pipe(catchError(this.handleError));
  }

  async getTenableAuditFileFilter(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/auditFile?filter=usable&fields=name`, { headers })
      .pipe(catchError(this.handleError));
  }

  async getTenableScanPolicyPluginsFilter(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/policy?filter=usable&fields=name`, { headers })
      .pipe(catchError(this.handleError));
  }

  async getTenableUsersFilter(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/user?fields=name,username,firstname,lastname`, { headers })
      .pipe(catchError(this.handleError));
  }

  async getTenablePluginFamily(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/pluginFamily?fields=name`, { headers })
      .pipe(catchError(this.handleError));
  }

  async getTenableRepositories(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/tenable/repository`, { headers })
      .pipe(catchError(this.handleError));
  }

  async postTenableSolutions(solutionParams: any): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.post(`${this.cpatApiBase}/tenable/solutions`, solutionParams, { headers })
      .pipe(catchError(this.handleError));
  }

  async postTenableSolutionAssets(solutionParams: any, solutionId: number): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.post(`${this.cpatApiBase}/tenable/solutions/${solutionId}/asset`, solutionParams, { headers })
      .pipe(catchError(this.handleError));
  }

  async postTenableSolutionVuln(solutionParams: any, solutionId: number): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.post(`${this.cpatApiBase}/tenable/solutions/${solutionId}/vuln`, solutionParams, { headers })
      .pipe(catchError(this.handleError));
  }
  ...
  async getIAVInfoForPlugins(pluginIDs: number[]): Promise<Observable<any>> {
    ...
    return this.http.get(`${this.cpatApiBase}/iav/pluginInfo`, { headers, params })
      .pipe(catchError(this.handleError));
  }

  async getIAVPluginIds(): Promise<Observable<any>> {
    const headers = await this.getAuthHeaders();
    return this.http.get(`${this.cpatApiBase}/iav/pluginIDs`, { headers })
      .pipe(catchError(this.handleError));
  }
}
