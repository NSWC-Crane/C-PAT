...
export class ImportService {
  ...
  private cachedGet<T>(url: string): Observable<T> {
    ...
    return this.isTenableProxy(url) ? this.cache.observe<T>(`${ImportService.TENABLE_CACHE_PREFIX}${this.cache.keyFor(url)}`, request, this.isTenableSuccess) : request();
  }
  ...
  private cachedPost<T>(url: string, body: any): Observable<T> {
    ...
    return this.isTenableProxy(url) ? this.cache.observe<T>(`${ImportService.TENABLE_CACHE_PREFIX}${this.cache.keyForBody(url, body)}`, request, this.isTenableSuccess) : request();
  }
