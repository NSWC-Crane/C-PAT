...
export class TenableVulnerabilitiesComponent implements OnInit {
  ...
  loadAssetOptions(): Observable<any> {
    return this.integrationService.getTenableAssetsFilter().pipe(
      map((assetsData) => {
  ...
  loadAuditFileOptions(): Observable<any> {
    return this.integrationService.getTenableAuditFileFilter().pipe(
      map((auditFileData) => {
  ...
  loadFamilyOptions(): Observable<any> {
    return this.integrationService.getTenablePluginFamily().pipe(
      map((familyData) => {
  ...
  loadScanPolicyPluginOptions(): Observable<any> {
    return this.integrationService.getTenableScanPolicyPluginsFilter().pipe(
  ...
  loadUserOptions(): Observable<any> {
    return this.integrationService.getTenableUsersFilter().pipe(
      map((userData) => {
      ...
      .pipe(
        ...
        switchMap((data) => {
          ...
          if (pluginIDs.length > 0) {
            return this.integrationService.getIAVInfoForPlugins(pluginIDs).pipe(
              catchError((error) => {
                this.messageService.add({
                  severity: 'error',
                  summary: 'Error',
