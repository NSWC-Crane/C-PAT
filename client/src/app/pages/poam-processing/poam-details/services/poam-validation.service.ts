...
export class PoamValidationService {
    constructor(private mappingService: PoamVariableMappingService) {}
    ...
    validateSubmissionRequirements(poam: any, teamMitigations: any[], poamMilestones: any[], dates: any): { valid: boolean; message?: string } {
        ...
        if (poam.isGlobalFinding) {
            ...
            const teamsWithoutMilestones = activeTeams.filter((activeTeam) => {
                return !poamMilestones.some((milestone) => (milestone.assignedTeamName === activeTeam.teamName || milestone.assignedTeamId === activeTeam.assignedTeamId) && milestone.milestoneComments && milestone.milestoneComments.trim() !== '');
            });
