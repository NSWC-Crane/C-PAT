...
interface FilteredPoamData {
  status?: { status: string; statusCount: number }[];
  label?: { label: string; labelCount: number }[];
  severity?: { severity: string; severityCount: number }[];
  scheduledCompletion?: { scheduledCompletion: string; scheduledCompletionCount: number }[];
  taskOrder?: { hasTaskOrder: boolean; count: number }[];
}
